...
for n = 2:1:length(t)-1
    ...
    % -- dx
    dx1_1(n) = y_sequential(n-1)*k3;
    dx1_2(n) = z_sequential(n-1)*k3;
    dx1_3(n) = x_sequential(n-1);
    ...
    dx4_1(n) = dx3_1(n-1); % dx4_1(n) = floor(dx3_1(n-1)*2^FRAC_PART)/(2^FRAC_PART); % rounding

    dx5_1(n) = dx4_1(n-1);

    % -- dy
    dy1_1(n) = y_sequential(n-1)*k4;
    dy1_2(n) = z_sequential(n-1)*k3;
    dy1_3(n) = x_sequential(n-1);
    ...
    dy4_1(n) = dy3_1(n-1); % dy4_1(n) = floor(dy3_1(n-1)*2^FRAC_PART)/(2^FRAC_PART); % rounding

    dy5_1(n) = dy4_1(n-1);  

    % -- dz
    dz1_1(n) = y_sequential(n-1) * k1;
    dz1_2(n) = z_sequential(n-1) * k5;
    dz1_3(n) = z_sequential(n-1) - 1;
    ...
    dz5_1(n) = dz4_1(n-1); %dz5_1(n) = floor(dz4_1(n-1)*2^FRAC_PART)/(2^FRAC_PART); % rounding
    ...
    % Sum
    %=====================================================================%
    x_sequential(n) = mul_reg_x(n-1) + x_sequential(n-1);  
    y_sequential(n) = mul_reg_y(n-1) + y_sequential(n-1); 
    z_sequential(n) = mul_reg_z(n-1) + z_sequential(n-1);
