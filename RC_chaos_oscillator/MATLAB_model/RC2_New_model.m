...
% heaviside_memory_12_bit_adress_space(heaviside_memory_12_bit_adress_space  > (TRUNCATION_Value -2^(-FRAC_PART_signal)) .*2^(mem_data_BW-8)) = (TRUNCATION_Value -2^(-FRAC_PART_signal)) .*2^(mem_data_BW-8); % -2^(-FRAC_PART) because 127.something is the most positive

heaviside_memory_12_bit_adress_space = min(max(heaviside_memory_12_bit_adress_space, -2^(mem_data_BW-1)),2^(mem_data_BW-1)-1);


% heaviside_memory_12_bit_adress_space_floored = round(heaviside_memory_12_bit_adress_space*2^FRAC_PART_signal)/(2^FRAC_PART_signal);
...
%=========================================================================%
%% Transform memory to hex and binary

for n = 1:length(heaviside_memory_12_bit_adress_space_floored)
    % Step 1: Scale the number to an integer
    scaled_num = round(heaviside_memory_12_bit_adress_space_floored(n)); % Scale by 2^14
...
fileID = fopen('Heaviside_memory_Hex.txt','w');
formatSpec = 'x"%s",\n';
...
fileID = fopen('Heaviside_memory_Bin.txt','w');
formatSpec = '"%s",\n';
...
figure(19)
hold on
plot(z_in_12_bit_adress_space,heaviside_memory_12_bit_adress_space / 2^(mem_data_BW-8),'LineWidth',2)
grid on, grid minor
xlabel("z_n (12-bit adress space)");
ylabel("Memory data");
set(gca, 'YLim', [-80, 140], 'YTick', -80:20:140);
